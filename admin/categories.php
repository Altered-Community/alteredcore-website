...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && csrfValid($_POST['csrf_token'] ?? '')) {
    ...
    if (($action === 'move_up' || $action === 'move_down') && $cid) {
        if (!adminCanEdit()) { flash('You do not have permission.', 'error'); redirect(BASE_URL . '/admin/categories'); }
        $cats = $db->query(q("SELECT id, sort_order FROM {news_categories} ORDER BY sort_order, name_en"))->fetchAll();
        ...
        if ($pos !== false) {
            ...
            if (isset($ids[$swapPos])) {
                ...
                $db->prepare(q("UPDATE {news_categories} SET sort_order = :s WHERE id = :id"))->execute([':s' => $sortB, ':id' => $cid]);
                $db->prepare(q("UPDATE {news_categories} SET sort_order = :s WHERE id = :id"))->execute([':s' => $sortA, ':id' => $swapId]);
            }
        }
    }
...
<div class="admin-header-bar">
    <h1><i class="fa-solid fa-tags me-2"></i>Categories</h1>
    <?php if (adminCanCreate()): ?>
    <a href="<?= BASE_URL ?>/admin/categories-edit" class="btn btn-sm btn-primary-altered">
            ...
            <?php else: ?>
                ...
                <tr>
                    ...
                    <td class="text-end" style="white-space:nowrap">
                        ...
                        <!-- Edit -->
                        <a href="<?= BASE_URL ?>/admin/categories-edit?id=<?= $row['id'] ?>"
                        ...
                        <a href="<?= BASE_URL ?>/admin/categories-delete?id=<?= $row['id'] ?>"
                           class="btn btn-sm btn-outline-danger" title="Delete"
                           onclick="return confirm('Delete this category? News using it will become uncategorized.')">
