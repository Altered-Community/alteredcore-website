...
if ($id) {
    $stmt = $db->prepare(q("SELECT * FROM {news_categories} WHERE id = :id"));
    $stmt->execute([':id' => $id]);
    $row  = $stmt->fetch();
    if ($row) {
        $cat = $row;
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!csrfValid($_POST['csrf_token'] ?? '')) {
        ...
        if (empty($errors)) {
            $stmt = $db->prepare(q("SELECT id FROM {news_categories} WHERE slug = :slug AND id != :id"));
        ...
        if (empty($errors)) {
            if ($id) {
                $db->prepare(q("UPDATE {news_categories} SET name_en=:en, name_fr=:fr, slug=:slug, is_hidden=:h WHERE id=:id"))
                   ->execute([':en'=>$nameEn,':fr'=>$nameFr,':slug'=>$slug,':h'=>$isHidden,':id'=>$id]);
                flash('Category updated.');
            } else {
                $db->prepare(q("INSERT INTO {news_categories} (name_en, name_fr, slug, is_hidden) VALUES (:en,:fr,:slug,:h)"))
...
<div class="admin-header-bar">
    <h1><i class="fa-solid fa-tag me-2"></i><?= $id ? 'Edit category' : 'Add category' ?></h1>
    <a href="<?= BASE_URL ?>/admin/categories" class="btn btn-outline-secondary btn-sm">← Back</a>
</div>
    ...
    <form method="post" novalidate>
        ...
        <div class="mb-4">
            ...
            <input type="text" name="slug" class="form-control font-monospace" id="slugField"
                   value="<?= h($cat['slug']) ?>" placeholder="e.g. game-updates">
        </div>
        ...
        <div class="mb-4">
            <div class="form-check form-switch">
                ...
                <label class="form-check-label" for="isHidden">
                    Hide this category <span class="text-muted small">(and all its articles) from the public site</span>
                </label>
            </div>
        </div>
        ...
        <div class="d-flex gap-2">
            ...
            <a href="<?= BASE_URL ?>/admin/categories" class="btn btn-sm btn-outline-secondary">Cancel</a>
        </div>
    </form>
</div>
