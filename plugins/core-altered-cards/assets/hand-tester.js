...
(function () {
    ...
    // toggle now — the playground is available on mobile too (with a tap action-sheet instead
    // of drag-and-drop; see openCardActions / onDown).
    function isSimple() { return !playgroundEnabled; }
    ...
    // Mobile: a bottom action-sheet replaces drag-and-drop (tap a card → pick what to do).
    function actionSheet(items) {
        var bd = document.createElement('div');
        bd.className = 'pt-sheet-backdrop';
        var sheet = document.createElement('div');
        sheet.className = 'pt-sheet';
        function close() {
            bd.classList.remove('pt-sheet-open');
            setTimeout(function () { if (bd.parentNode) bd.parentNode.removeChild(bd); }, 220);
        }
        items.forEach(function (it) {
            var b = document.createElement('button');
            b.type = 'button';
            b.className = 'pt-sheet-btn' + (it.cancel ? ' pt-sheet-cancel' : '');
            b.innerHTML = (it.icon ? '<i class="' + it.icon + '"></i>' : '') + '<span>' + it.label + '</span>';
            b.addEventListener('click', function (e) { e.stopPropagation(); close(); if (it.fn) it.fn(); });
            sheet.appendChild(b);
        });
        bd.appendChild(sheet);
        bd.addEventListener('click', function (e) { if (e.target === bd) close(); });
        document.body.appendChild(bd);
        requestAnimationFrame(function () { bd.classList.add('pt-sheet-open'); });
    }
    function openCardActions(card, zone) {
        var items = [];
        if (zone === 'board') {
            items.push({ label: handTxt.returnHand, icon: 'fa-solid fa-rotate-left', fn: function () { moveCard(card.id, 'hand'); } });
        } else {
            items.push({ label: handTxt.playBoard, icon: 'fa-solid fa-bolt', fn: function () { moveCard(card.id, 'board'); } });
        }
        items.push({ label: handTxt.toMana,  icon: 'fa-solid fa-droplet', fn: function () { moveCard(card.id, 'mana'); } });
        items.push({ label: handTxt.discard, icon: 'fa-solid fa-trash',   fn: function () { moveCard(card.id, 'discard'); } });
        items.push({ label: handTxt.zoom,    icon: 'fa-solid fa-magnifying-glass', fn: function () { openZoom(card); } });
        items.push({ label: handTxt.cancel,  cancel: true });
        actionSheet(items);
    }
    ...
    handEl.addEventListener('click', function (e) {
        ...
        if (!card) return;
        if (isSimple()) { openZoom(card); return; }                     // playground off: tap = zoom
        if (state.phase === 'setup') { toggleManaSelect(card); return; }  // flip-select 3 for mana (tap)
        if (isMobile()) { openCardActions(card, 'hand'); return; }        // mobile play: tap → action sheet
        // desktop play phase: drag-and-drop handles the moves
    });
    if (boardCardsEl) boardCardsEl.addEventListener('click', function (e) {
        if (justDragged) { justDragged = false; return; }
        if (e.target.closest('.pt-loupe') || e.target.closest('.pt-tomana')) return;
        var wrap = e.target.closest('.pt-card');
        if (!wrap || !boardCardsEl.contains(wrap)) return;
        var card = state.board.filter(function (c) { return c.id === wrap.dataset.id; })[0];
        if (card && isMobile() && state.phase === 'play') openCardActions(card, 'board');
    });
    ...
    function setupDrag() {
        ...
        function onDown(e) {
            ...
            if (isSimple() || isMobile() || state.phase !== 'play') return;   // desktop play-phase only (mobile uses the tap action-sheet)
            if (e.target.closest('.pt-loupe') || e.target.closest('.pt-tomana')) return;
